...
class MRCData(data.Dataset):
    ...
    '''
    def __init__(self, mrcfile, norm=None, keepreal=False, invert_data=False, ind=None, window=True, datadir=None, max_threads=16, window_r=0.85, flog=None, eval_z=False, seq_len=10):
        log = flog if flog is not None else utils.log
        if keepreal:
    ...
    def __getitem__(self, index):
        ...
        start1 = index 
        end1 = self.N 
        start2 = index % (self.N // self.seq_len) 
        end2 = index 
        result1 = np.concatenate((self.particles[start1:end1:(self.N // self.seq_len)],self.particles[start2:end2:(self.N // self.seq_len)]),axis=0) 
        result2 = list(range(start1,end1,self.N // self.seq_len)) + list(range(start2,end2,self.N // self.seq_len)) 
        
        #if self.eval_z == False:
        #    rand = np.random.randn()
        #    if rand > 0:
        #        result1 = np.flipud(result1).copy()
    ...
    def __getitem__(self, index):
        ...
        if self.eval_z == True:
            ...
            start1 = index # cas9
            end1 = self.N # cas9
            start2 = index % (self.N // self.seq_len) # cas9
            end2 = index # cas9
            result1 = np.concatenate((self.particles[start1:end1:(self.N // self.seq_len)],self.particles[start2:end2:(self.N // self.seq_len)]),axis=0) # cas9
            result2 = list(range(start1,end1,self.N // self.seq_len)) + list(range(start2,end2,self.N // self.seq_len)) # cas9
            
            start = index # 7bcq
            end = index + self.seq_len # 7bcq
            result1 = self.particles[start:end] # 7bcq
            result2 = range(start,end) # 7bcq
            
            # 7bcq_cycle
            start1 = index # 7bcq_cycle
            end1 = self.N # 7bcq_cycle
            start2 = index % (self.N // self.seq_len) # 7bcq_cycle
            end2 = index # 7bcq_cycle
            result1 = np.concatenate((self.particles[start1:end1:(self.N // self.seq_len)],self.particles[start2:end2:(self.N // self.seq_len)]),axis=0) # 7bcq_cycle
            result2 = list(range(start1,end1,self.N // self.seq_len)) + list(range(start2,end2,self.N // self.seq_len)) # 7bcq_cycle
            
            return result1, result2

        # FIXME randomly flip the tensor _YEZ
