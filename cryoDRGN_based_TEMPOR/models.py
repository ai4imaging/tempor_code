...
class CryodrgnVAE(nn.Module):
    ...
    def __init__(self, lattice, # Lattice object
        ...
        super(CryodrgnVAE, self).__init__()
    ...
    def load(self, config, weights=None, device=None):
        ...
        # Returns:
        #     CryodrgnVAE instance, Lattice instance
        '''
...
class HetOnlyVAE(nn.Module):
    ...
    def __init__(self, lattice, # Lattice object
        ...
        self.mlp_x_to_h = ResidLinearMLP(in_dim, 3, 1024, hDim, activation) 
        self.rnn_h_to_g = nn.LSTM(hDim, gDim, num_layers, batch_first=True, bidirectional=bidirectional)
        self.mlp_h_to_z0 = ResidLinearMLP(in_dim, 3, 1024, zdim * 2, activation) 
        
        self.mlp_z_to_g = nn.Sequential(
            nn.Linear(zdim, gDim),
            nn.ReLU(),
            nn.Linear(gDim, gDim),
			nn.ReLU(),
        ...
        self.inf_mean = nn.Linear(gDim, zdim)
        self.inf_logvar = nn.Linear(gDim, zdim)

        self.mlp_z_prop = nn.Sequential(nn.Linear(zdim, kDim), nn.ReLU(), nn.Linear(kDim, kDim), nn.ReLU(), nn.Linear(kDim, zdim))
        self.mlp_z_prop_gate = nn.Sequential(nn.Linear(zdim, kDim), nn.ReLU(), nn.Linear(kDim, kDim), nn.ReLU(), nn.Linear(kDim, zdim))
		
        self.mlp_z_prop_mean = nn.Sequential(nn.Linear(zdim, kDim), nn.ReLU(), nn.Linear(kDim, kDim), nn.ReLU(), nn.Linear(kDim, zdim))
        self.mlp_z_prop_std = nn.Sequential(nn.Linear(zdim, kDim), nn.ReLU(), nn.Linear(kDim, kDim), nn.ReLU(), nn.Linear(kDim, zdim), nn.Softplus())
    ...
    def inference_DKF(self, x):
        ...
        x = x.view(batch_size, seq_len, in_dim)
        ...
        in_dim_ = 3208 # FIXME _YEZ
        x_ = torch.zeros((batch_size, seq_len, in_dim_)).to(device_)
        for i in range(batch_size):
            for j in range(seq_len):
                x_[i, j, :] = x[i, j, self.enc_mask]
        x = x_
		
        h = self.mlp_x_to_h(x)
        if self.bidirectional:
            g, _ = self.rnn_h_to_g(h)
            g = g.view((-1, self.seq_len, 2, self.gDim))
            g_forward = g[:, :, 0, :]
            g_backward = g[:, :, 1, :]
        else:
            g, _ = self.rnn_h_to_g(torch.flip(h, [1]))
            g = torch.flip(g, [1])
        ...
        z_mean_list = []
        z_logvar_list = []
        # z0_hid = self.mlp_h_to_z0(h.view((-1, 1, self.seq_len*self.hDim)))
        z0_hid = self.mlp_h_to_z0(x_[:, 0, :].view(batch_size, 1, in_dim_)) 
        z0_mean = z0_hid[:,:,:self.zdim]
        z0_logvar = z0_hid[:,:,self.zdim:]
        z0 = self.reparameterize(z0_mean, z0_logvar)
        z_list.append(z0)
        z_mean_list.append(z0_mean)
        z_logvar_list.append(z0_logvar)
        for t in range(self.seq_len): 
            if self.bidirectional:        
                gt = (self.mlp_z_to_g(z_list[-1]) + g_forward[:, t:t+1, :] + g_backward[:, t:t+1, :]) / 3
                # gt = (self.mlp_z_to_g(z_list[-1]) + g_backward[:, t:t+1, :]) / 2 # DEEP kalman smoother 
                # gt = (self.mlp_z_to_g(z_list[-1]) + g_forward[:, t:t+1, :]) / 2 # DEEP kalman filter 
                # gt = (g_forward[:, t:t+1, :] + g_backward[:, t:t+1, :]) / 2 # MF-LR 
            else:
                gt = (self.mlp_z_to_g(z_list[-1]) + g[:, t:t+1, :]) / 2
            zt_hid = self.mlp_g_to_z(gt)
            zt_mean = self.inf_mean(zt_hid)
            zt_logvar = self.inf_logvar(zt_hid)
            zt = self.reparameterize(zt_mean, zt_logvar)
            z_list.append(zt)
            z_mean_list.append(zt_mean)
            z_logvar_list.append(zt_logvar)
            
        z = torch.cat(z_list, 1)
        z_mean = torch.cat(z_mean_list, 1)
        z_logvar = torch.cat(z_logvar_list, 1)
    ...
    def transition(self, z_tm1):
        gate = self.mlp_z_prop_gate(z_tm1)
        z_prop = self.mlp_z_prop(z_tm1)
        z_mean_p = (1 - gate) * self.mlp_z_prop_mean(z_tm1) + gate * z_prop
        z_logvar_p = torch.log(self.mlp_z_prop_std(z_tm1))
